...
static int ws1508_sdio_init(unsigned port)
{
	switch (port) {
		case SDIO_PORT_A:
			break;
...
static void ws1508_sdio_pwr_prepare(unsigned port)
{
	cpu_sdio_pwr_prepare(port);
}

static void ws1508_sdio_pwr_on(unsigned port)
{
	return;
}

static void ws1508_sdio_pwr_off(unsigned port)
{
	return;
}

static int ws1508_sdio_detect(unsigned port)
{
	switch (port) {
		case SDIO_PORT_A:
			break;
...
static void board_mmc_register(unsigned port)
{
	...
	aml_priv->sdio_init			 = ws1508_sdio_init;
	aml_priv->sdio_pwr_prepare	 = ws1508_sdio_pwr_prepare;
	aml_priv->sdio_pwr_on	 = ws1508_sdio_pwr_on;
	aml_priv->sdio_pwr_off	 = ws1508_sdio_pwr_off;
	aml_priv->sdio_detect		 = ws1508_sdio_detect;

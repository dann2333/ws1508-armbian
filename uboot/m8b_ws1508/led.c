...
static LED_ID ws1508_led_getId(const char *label)
{
	LED_ID id;
...
static int ws1508_led_getPin(LED_ID id)
{
	if(id == LED_ID_RED)
		return GPIOAO_2;
...
static LED_STATE ws1508_led_get(LED_ID id)
{
	int pin = ws1508_led_getPin(id);
	if(pin < 0)
...
static int ws1508_led_set(LED_ID id, LED_STATE state)
{
	int pin = ws1508_led_getPin(id);
	if(pin < 0)
		return pin;
	...
	if(state == LED_STATE_TOGGLE)
		state = (ws1508_led_get(id) == LED_STATE_ON) ? LED_STATE_OFF : LED_STATE_ON;
...
void ws1508_led_init(void)
{
	ws1508_led_set(LED_ID_RED, LED_STATE_ON);
	ws1508_led_set(LED_ID_GREEN, LED_STATE_OFF);
	ws1508_led_set(LED_ID_BLUE, LED_STATE_OFF);
}
...
int do_led(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if(strcmp(argv[1], "list") == 0) {
		...
		LED_ID id;
		for(id = 0; id < LED_ID_INVALID; id++) {
			LED_STATE state = ws1508_led_get(id);
			printf("%s\t%s\n", led_id_lable[id], led_state_label[state]);
		}
		return 0;
	}

	LED_ID id = ws1508_led_getId(argv[1]);
	...
	for(state = 0; state < LED_STATE_INVALID; state++)
		if(strcmp(argv[2], led_state_label[state]) == 0)
			return ((ws1508_led_set(id, state) >= 0) ? 0 : 1);

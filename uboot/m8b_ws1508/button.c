...
static BUTTON_ID ws1508_button_getId(const char *label)
{
	BUTTON_ID id;
...
static int ws1508_button_getPin(BUTTON_ID id)
{
...
static BUTTON_STATE ws1508_button_get(BUTTON_ID id)
{
	int pin = ws1508_button_getPin(id);
	if(pin < 0)
...
void ws1508_button_init(void)
{
	ws1508_button_get(BUTTON_ID_RESET);
}
...
int do_button(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if(strcmp(argv[1], "list") == 0) {
		...
		BUTTON_ID id;
		for(id = 0; id < BUTTON_ID_INVALID; id++) {
			BUTTON_STATE state = ws1508_button_get(id);
			printf("%s\t%s\n", button_id_lable[id], button_state_label[state]);
		}
		return 0;
	}

	BUTTON_ID id = ws1508_button_getId(argv[1]);

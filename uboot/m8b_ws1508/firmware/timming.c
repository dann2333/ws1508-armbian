/*
 * DDR timing / PLL setup for the Xunlei WS1508.
 *
 * Copied verbatim from board/amlogic/m8b_onecloud/firmware/timming.c
 * (syb999/uboot-onecloud). It is deliberately unchanged: the S805 DDR
 * initialiser auto-detects both the bus width (ddr_mode_auto_detect) and
 * the density (ddr_size_auto_detect probes 256MB/512MB/1GB), so this one
 * timing set covers the WS1508's 512MB as well as the OneCloud's 1GB.
 * Only the board name in the #error strings was updated.
 */
...
static int init_pctl_ddr3(struct ddr_set *timing_set);
	...
	#else
		#error "Over PLL range! Please check CONFIG_DDR_CLK with file m8b_ws1508.h! \n"
	#endif
#else
	...
	#else
		#error "Over PLL range! Please check CFG_DDR_CLK with file m8b_ws1508.h! \n"
	#endif
#endif
...
static struct ddr_set __ddr_setting = {
	...
	.t_ddr_apd_ctrl =	(32 << 0) | // [B7..B0]: DMC active latency.  latency to enter LOW POWER state after the the DMC is not actived.
						...
						( 0 << 16), // [B17,B16]: DDR mode to genreated the refresh ack signal and prcharge signal to DMC for dmc refresh control.
						            // 2'b00: DDR3
						            // 2'b01: LPDDR2/3
						            // 2'b10, 2'b11: no refresh ack and precharge signals generated.
	...
	.t_pctl_mcfg =	(1 <<  0) |  // [B0] mem-bl: 0 for 4; 1 for 8
					...
					(((((CFG_DDR_FAW + CFG_DDR_RRD - 1) / CFG_DDR_RRD) - 4) & 0x3) << 18) |
					             // [B19,B18] tfaw_cfg
	...
	.t_pctl_mcfg1 = (1 << 31) | // [B31]hw_exit_idle_en
					(((CFG_DDR_FAW % CFG_DDR_RRD) ? (CFG_DDR_RRD - (CFG_DDR_FAW % CFG_DDR_RRD)) : 0) & 0x7) << 8,
					            // [B10,B9,B8] tfaw_cfg_offset:
	...
	.t_pub_mr = {
		[0] =	(1 << 12) | // [B12] 1 fast exit from power down , 0 slow
				((((CFG_DDR_WR <= 8) ? (CFG_DDR_WR - 4) : (CFG_DDR_WR >> 1)) & 7) << 9) |
				            // [B11,B10,B9] WR recovery. It will be calcualted by get_mrs0()@ddr_init_pctl.c
				(0 <<  8) | // [B8] DLL reset
				(0 <<  7) | // [B7]
				            //   0=Normal
				            //   1=Test.
				(((CFG_DDR_CL - 4) & 0x7) << 4) |
				            // [B6,B5,B4] CL cas latency high 3 bits (B6,B5,B4)
				(((CFG_DDR_CL - 4) & 0x8) >> 1) |
				            // [B2] CL bit: 0
				(0 <<  3) | // [B3] burst type
							//   0:sequential
...
#endif
				((CFG_DDR_AL ? ((CFG_DDR_CL - CFG_DDR_AL) & 3) : 0) << 3),
				                                 // [B4,B3] AL:
				                                 //   00: AL disabled
				                                 //   01: CL-1
				                                 //   10: CL-2
...
//#endif
	...
	.t_pub_ptr = {
		[0] =	(6   << 0) | // PHY RESET(APB clock cycles)
				(320 << 6) | // tPLLGS(APB clock cycles): 4us
				(80 << 21),  // tPLLPD(APB clock cycles): 1us

		[1] =	(120  << 0) |  // tPLLRST(APB clock cycles): 9us
				(1000 << 16),  // TPLLLOCK(APB clock cycles): 100us

		[2] = 0,

		[3] =	(20000 <<  0)|  // tDINIT0: CKE low time with power and lock stable: 500us.
				(186   << 20),  // tDINIT1: CKE high time to first command(tRFC + 10ns)

		[4] =	(10000 <<  0) | // tDINIT2: RESET low time: 200us
				(800   << 18),  // tDINIT3: ZQ initialization command to first command: 1us
	...
	.t_mmc_ddr_ctrl =	(1                           << 16) | // [B16]: bank page policy
						(CONFIG_M8B_DDR_BIT_MODE_SET << 15) | // [B15]: rank1 bit mode
						                                      //   0 : 32bits mode
						                                      //   1: 16bits mode
						(CONFIG_M8B_DDR_BANK_SET     << 13) | // [B14-B13]: rank1 address map bank mode
						...
						(CONFIG_M8B_DDR_RANK_SET     << 12) | // [B12]
						                                      //   0: one rank
						                                      //   1: two ranks.
						(CONFIG_DDR3_ROW_SIZE        << 10) | // [B11-10]: rank1 row size
						                                      //   01: A0~A12
						                                      //   10: A0~A13
						                                      //   11: A0~A14
						                                      //   00: A0~A15
						(CONFIG_DDR3_COL_SIZE        <<  8) | // [B9-B8]: rank1 col size
						                                      //   01: A0~A8
						                                      //   10: A0~A9
						(CONFIG_M8B_DDR_BIT_MODE_SET <<  7) | // [B7]: ddr rank0 bit mode
						                                      //   0: 32bits mode
						                                      //   1 : 16bits mode
						(CONFIG_M8B_DDR_BANK_SET     <<  5) | // [B6-B5]: rank0 address map bank mode
						                                      //   00: address switch between 2 banks  bank[0] selection bits [12].
						                                      //   01: address switch between 4 banks  bank[1:0] selection bits [13:12].
						                                      //   10: address switch between 2 banks  bank[0] selection bits [8].
						                                      //   11: address switch between 4 banks  bank[1:0] selection bits [9:8].
						(CONFIG_M8B_DDR_RANK_SET     <<  4) | // [B4]
						                                      //   0: one rank
						                                      //   1: two ranks.
						(CONFIG_DDR3_ROW_SIZE        <<  2) | // [B3-B2]: rank0 row size
						                                      //   01: A0~A12
						                                      //   10: A0~A13
						                                      //   11: A0~A14
						                                      //   00: A0~A15
						(CONFIG_DDR3_COL_SIZE        <<  0),  // [B1-B0]: rank0 col size
						                                      //   01: A0~A8
						                                      //   10: A0~A9
	...
	.t_mmc_ddr_timming0 =	(((CFG_DDR_CWL + 1) / 2 - 1) << 28) | // [B31-B28]: tCWL
							(((CFG_DDR_RP + 1) / 2 - 1)  << 24) | // [B27-B24]: tRP
							(((CFG_DDR_RTP + 1) / 2 - 1) << 20) | // [B23-B20]: tRTP
							(((CFG_DDR_RTW + 1) / 2 - 1) << 16) | // [B19-B16]: tRTW
							(4                           << 12) | // [B15-B12]: tCCD // where is the tCCD, not found in PUB/PCTL spec, 4 from ucode
							(((CFG_DDR_RCD + 1) / 2 - 1) <<  8) | // [B11-B8]: tRCD
							(((CFG_DDR_CL + 1) / 2 - 1)  <<  4) | // [B7-B4]: tCL
							(3                           <<  0),  // [B3-B0]: Burst length

	.t_mmc_ddr_timming1 =	(((CFG_DDR_CL + CFG_DDR_RCD + CFG_DDR_RP + 1) / 2 - 1) << 24) |
							                                      // [B31-B24]: tMISS: the waiting clock cycles for a miss page access (the same bank different page is actived. = tCL + tRCD + tRP)
							(6                           << 20) | // [B23-B20]: cmd hold number for read command
							(0                           << 16) | // [B19-B16]: cmd hold number for write command
							(((CFG_DDR_RFC + 1) / 2 - 1) <<  8) | // [B15-B8]: tRFC. refresh to other command time
							(((CFG_DDR_WTR + 1) / 2 - 1) <<  4) | // [B7-B4]: tWTR
							(((CFG_DDR_WR + 1) / 2 - 1)  <<  0),  // [B3-B0]: tWR

	.t_mmc_ddr_timming2 =	(((CFG_DDR_ZQCL + 1) / 2 - 1)     << 24) | // [B31-B24]: tZQCL
							(16                               << 16) | // [B23-B16]: tPVTI
							(((CFG_DDR_REFI + 1) / 2 - 1)     <<  8) | // [B15-B8]: tREFI
							(((CFG_DDR_CLK / 10 + 1) / 2 - 1) <<  0),  // [B7-B0]: t100ns

	.t_mmc_arefr_ctrl =	(0 << 9) | // [B9]: hold dmc command after refresh cmd sent to DDR3 SDRAM
						(0 << 8) | // [B8]: hold dmc command when dmc need to send refresh command to PCTL
						(1 << 7) | // [B7]: dmc to control auto_refresh enable
						(0 << 4) | // [B6-B4]: refresh number per refresh cycle
						(1 << 3) | // [B3]: pvt enable
						(1 << 2) | // [B2]: zqc enable
						(1 << 1) | // [B1]: ddr1 auto refresh dmc control select
						(1 << 0),  // [B0]: ddr0 auto refresh dmc control select
...
STATIC_PREFIX_DATA struct pll_clk_settings __plls = {
	...
	.sys_pll_cntl =	(M8_SYS_PLL_OD << 16) | // OD
					(M8_SYS_PLL_N  <<  9) | // N
					(M8_SYS_PLL_M  <<  0) | // M
					(2             << 20) | // SYS_DPLL_SS_CLK
					(1             << 24) | // SYS_DPLL_SS_AMP
					(0             << 28) | // SYS_DPLL_SSEN
					(1             << 29) | // SYS_DPLL_RESET
					(1             << 30),  // SYS_DPLL_EN
	// A9 clock setting
	...
	// clk81=fclk_div4/(3+1)=(2.55G/4)/(3+1)=159.375MHz
    // 0x105d [0xc1104174]
	.mpeg_clk_cntl =	(5 << 12) | // [B14,B13,B12]: MPEG_CK_SEL
						            //   0: XTAL/32KHz
						            //   1: ddr_pll
						            //   2: mpll_clk_out0
						            //   3: mpll_clk_out1
						            ...
						            //   6: fclk_div3
						            //   7: fclk_div5
						(0 << 9) |  // [B9] XTAL or 32KHz
						            //   0: 24MHz
						            //   1:32KHz
						(1 << 8) |  // [B8]
						            //   0: clk81=(XTAL or 32KHz)
						            //   1: clk81=pll
